...
class TestLogin(unittest.TestCase):
    ...
    def test_a_success_login(self):
       ...
       # Klik switch button "Create Login Details"
        ...
        # Memastikan radio button saat ini dalam keadaan enable
        radio_button = self.driver.find_element(By.CSS_SELECTOR, 'span.oxd-radio-input.oxd-radio-input--active')
        is_radio_enabled = "oxd-radio-input--active" in radio_button.get_attribute("class")

        # Jika radio button belum dalam keadaan enable, kita perlu mengkliknya terlebih dahulu
        if not is_radio_enabled:
            radio_button.click()
            time.sleep(2)

        # Mengklik radio button untuk mengubah statusnya dari enable menjadi disable
        radio_button.click()
        time.sleep(2)
        ...
        self.driver.find_element(By.XPATH, "//input[contains(@class, 'oxd-input') and contains(@class, 'oxd-input--active') and contains(@class, 'oxd-input--error')]").send_keys("asustuffff")
        time.sleep(3)
